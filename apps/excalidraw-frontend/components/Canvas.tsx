...
function Topbar({selectedTool, setSelectedTool}:{selectedTool:Tool, setSelectedTool:(s:Tool)=>void}){
    return (
        <div className="fixed  flex gap-2 w-fit p-2 left-0 right-0 text-red-200">
            ...
            <Icon icon={<Eraser />} onClick={()=>{setSelectedTool("eraser")}} activated={selectedTool === "eraser"} />
            <Icon icon={<Move />} onClick={()=>{setSelectedTool("panning")}} activated={selectedTool === "panning"} />
        </div>
    )
}